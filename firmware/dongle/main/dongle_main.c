...
static void send_info_response(int request_id)
{
    ...
    cJSON_AddNumberToObject(root, "id", request_id);
    cJSON_AddBoolToObject(root, "ok", true);
    cJSON_AddStringToObject(root, "device", "lilygo-t-dongle-s3");
    cJSON_AddStringToObject(root, "chip", "esp32s3");
    cJSON_AddStringToObject(root, "role", role_to_string(NODE_ROLE_IDENTITY));
    cJSON_AddStringToObject(root, "profile", "identity-token");
    cJSON_AddStringToObject(root, "version", APP_VERSION);
    cJSON_AddStringToObject(root, "transport", "usb-serial-jtag");
...
{
    int result = 0;
    static const char *personalization = "esp-messenger-dongle";
...
static void send_sign_bytes_response(int request_id, cJSON *request)
{
    ...
    if (!digest_ready) {
        mbedtls_ecdsa_free(&ecdsa);
        free_rng(&entropy, &ctr_drbg);
        send_error_response(request_id, "crypto_error", "Failed to hash payload");
        return;
    }
...
void app_main(void)
{
    ...
    ESP_LOGI(TAG, "ESP messenger dongle starting");
